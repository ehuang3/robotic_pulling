...
\begin{align}
  ...
  h_{i+1} &= h_i + d_i,
\end{align}
where the functions $\hat{\alpha}$ and $\hat{\beta}$ are fourer series
approximations of the upper and lower angular velocity bounds (to
avoid costly in-loop computations) and the additional state $h_i$
...
optimizer using paths generated from Dubin's curves. Pushing with
...
discarded the four trials where our planner failed trajectories that
satisfied workspace constraints. The average absolute displacement
from the target pose was 4.00mm $\pm$ 3.02mm. The average absolute
angular displacement from the target pose was 4.35 degrees $\pm$ 3.14
