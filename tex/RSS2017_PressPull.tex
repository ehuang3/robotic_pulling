...
contributions. Subsection \ref{sec:planar-pulling} serves two
...
used for reasoning about frictional torques. This construct is
particularly relevant for Subsection \ref{sec:point-in-moment-hull},
which introduces an algorithm that computes the feasibility of a given
angular velocity.
...
\subsection{Planar Pushing Subject to Friction}\label{sec:planar-pulling}
...
Our presentation follows that of \cite{alexander1993bounds, Mason}.
...
Recall from Section \ref{sec:background} that an angular velocity

feasibility of a given angular velocity 

In the quasi-static model 

Given a support region $R$ and a velocity $\mathbf{v}^+$, with angular
component $\omega$, at the contact point, the moment hull of $G(R)$ 

Recall from Subsection \ref{sec:frictional-envelope} that the moment
hull is the set of all feasible centers of pressure and frictional
moments for a given support region $R$ and velocity $\mathbf{v}^+$.

In the case of pushing with a single contact point, 


Under the quasi-static model of pushing, 

The moment hull is the set of all 

The moment hull is the set of all feasible centers of 
