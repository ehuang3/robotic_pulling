...
\begin{abstract}
  ...
  object's motion for classes of pressure distributions where the
  center of pressure is known but the distribution of support forces
  is unknown. The second contribution is to show that the exact motion
  bounds can be used to plan robotic pulling trajectories that bound
  the uncertainty in object pose during execution and guarantee
  convergence to the final pose.
...
\begin{figure}
\begin{center}
  \includegraphics[width=\columnwidth]{fig/object.png}
\end{center}
\caption{The top and bottom view of the cuboidal object with motion capture markers and 8 holes for pulling.}
\label{fig:object}
\end{figure}
...
The slope ($k$) of the Pseudo-Huber Loss function for the DDP planner was set to (5000, 5000, 1000, 1000, 40) and the width ($\lambda$) was set to (0.1, 0.1, 0.2, 0.2, 0.0) for the states $(x\,, y\,, u\,, l\,, d)$ respectively (where d is the total distance travelled.)
The planner was evaluated for all contact points and the trajectory with lowest cost while satisfying the workspace constraint was picked and executed on the robot at 25mm/s linear speed. The final pose was then recorded by the motion capture system. 
