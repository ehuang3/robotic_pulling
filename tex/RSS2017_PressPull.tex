...
\begin{theorem}\label{thm:angular-velocities}
  ...
  the set of all realizable angular velocities is connected.
\end{theorem}

\begin{proof}
  Let $\Omega$ be the set of realizable angular velocities,
...
\begin{corollary}
  For pulling of a planar rigid body with known center of pressure,
  the set of all realizable angular velocities is a bounded interval.
\end{corollary}
...
$[x_0,y_0]^T$. The run-time of the algorithm is
$\mathcal{O}(d\,n\log{} n)$, where $d$ is the number of significant
digits returned and $n$ is the number of points in the discretization
of $R$.
...
\DoWhile{$[x_0,y_0]^T \in \Call{ConvHull}{G}$}
...
\State $G \gets \{-\mathbf{x}\times A(\mathbf{x})\mathbf{v}^+ / \lVert A(\mathbf{x})\mathbf{v}^+ \rVert \;|\; \mathbf{x} \in R\}$
\If {$[x_0,y_0]^T \in \Call{ConvHull}{G}$} 
\State $u \gets \omega$
\Else{} 
\State $l \gets \omega$
\EndIf
