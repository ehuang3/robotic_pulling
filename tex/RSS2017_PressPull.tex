...
Fig.~\ref{fig:hardware} shows the experimental setup to we used to
...
4 trials were the algorithm failed to find any feasible trajectory
within the robot workspace. We distinguish between two types of error
during the pulling experiments. The target error is the error between
the target pose and the final pose measured by the MoCap. The
commanded error is the error between the last commanded pose and the
final pose measured by the MoCap. 
...
\caption{(Dashed red line) The planned robotic pulling
  ...
  rectangles) The measure poses of the object when the trajectory was
