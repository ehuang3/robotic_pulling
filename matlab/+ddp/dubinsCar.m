...
p2 = rand(2,1) + [1;0]
r2 = rand
s = @(p,r,t) [p(1)+r*cos(t);p(2)+r*sin(t)];
s1 = s(p1,r1,rand*2*pi);
s2 = s(p2,r2,rand*2*pi);
d1 = randi(2)-1
d2 = randi(2)-1
...
for i = 1:2:size(T,2)
    ...
    S = repmat(t,[2,1]).*repmat(t1,[1,round(norm(t2-t1)/step)]) + repmat((1-t),[2,1]).*repmat(t2,[1,round(norm(t2-t1)/step)]);
    traj = [traj S];
