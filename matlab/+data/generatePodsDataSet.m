...
pod = randomPod(2, r_max, p_mean, p_sigma)
V = pod.V
K = pod.K
X = [V(1,K(:,1)); V(1,K(:,2))];
Y = [V(2,K(:,1)); V(2,K(:,2))];
...
grid on
axis equal
X = [pod(1,K(:,1)); pod(1,K(:,2))];
Y = [pod(2,K(:,1)); pod(2,K(:,2))];
plot(X,Y,'b')
plot(r_max*cos(t), r_max*sin(t),'k')
...
% % Sample random ellipse.
% e_max = r_max - norm(pt);
% a = rand*e_max;
% b = rand*e_max;
% t = linspace(0,2*pi);
% rot = @(t) [cos(t) -sin(t); sin(t) cos(t)];
% pts = rot(rand*2*pi) * [a*cos(t); b*sin(t)];
% pts = pts + repmat(pt, [1,length(t)]);

% Sample random circle.
e_min = (r_max - r_mean)/2/n_feet
e_max = r_max - norm(pt);
r = rand*(e_max-e_min) + e_min;
t = linspace(0,2*pi);
pts = r.*[cos(t); sin(t)];
pts = pts + repmat(pt, [1,length(t)]);
